"""
@FileName: td_psola.py
@Description: Implement td_psola
@Author: Ryuk
...
from utils import auto_correlation
from filter.fir_filter import fir_lpf

def process(x, fs, pitch_scale, time_scale, cutoff_freq=500):
    # normalize
    x = x - np.mean(x)
    x = x / np.max(np.abs(x))

    #x = LowPassFilter(x, fs, cutoff_freq)
    pitch = pitch_estimator(x, fs)
    output = pitch_mark(x, pitch, fs, pitch_scale, time_scale)
    return output


def pitch_estimator(x, fs):
    ...
    for count in range(1, frame_num):
        frame = x[frame_range]
        frame_pitch[count] = ptich_detection(frame, fs)
...
def center_clipping(x, clip_rate):
...
def ptich_detection(x, fs):
    min_lag = round(fs / 500)
    max_lag = round(fs / 70)
    x = center_clipping(x, 0.3)

    auto_corr = auto_correlation(x, max_lag)
    ...
    if max_corr > 0.35 and min_corr < 0 and is_peak(max_corr_index, min_lag, max_lag, auto_corr):
...
def find_peak_candidates(x, MaxCandidateNumber, Offset):
    length = len(x)
    x1 = np.roll(x, 1)
    x2 = np.roll(x, -1)
    PeakIndices = []
...
def increase_marking(x, p, fs, m):
    ...
    if i < length:
        while i < length:
            ...
            Offset = Range[0]
            c = find_peak_candidates(x[Range], m, Offset)
...
def VoicedSegmentMarking(x, p, fs):
    ...
    RightCandidates = increase_marking(x[i:length], p[i:length], fs, MaxCandidateNumber)
    LeftCandidates = increase_marking(np.flipud(x[:i+1]), np.flipud(p[: i+1]), fs, MaxCandidateNumber)
...
def pitch_mark(x, pitch, fs, pitch_scale, time_scale):
