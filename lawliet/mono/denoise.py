...
import numpy as np
from utils import nextpow2, calculate_snr
...
class SpectralSubtraction:
    ...
    def getNoiseSpectrum(self):
        ...
        for i in range(self.noise_frames):
            ...
            noise_spectrum += np.abs(np.fft.rfft(frame, self.nfft))
    ...
    def simpleSpectralSubtraction(self):
        ...
        for i in range(frames_nums):
            ...
            spectrum = np.fft.rfft(frame, self.nfft)
            ...
            frame = np.fft.irfft(sub_speech_spectrum, self.nfft)
    ...
    def BeroutiSpectralSubtraction(self):
        ...
        for i in range(frames_nums):
            ...
            snr = calculate_snr(magnitude, noise_spectrum)
            alpha = self.get_alpha(snr)
